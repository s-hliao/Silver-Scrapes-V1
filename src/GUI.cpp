...
GUI::state_t::state_t(QLength ix, QLength iy, QAngle itheta) : x(ix), y(iy), theta(itheta) {}

GUI::sensors_t::sensors_t(double ileft, double iright) : left(ileft), right(iright), hasMiddle(false) {}

GUI::sensors_t::sensors_t(double ileft, double iright, double imiddle) : left(ileft), right(iright), middle(imiddle), hasMiddle(true) {}
...
: GUI(parent, lv_obj_get_style(parent)->body.main_color) {}
 ...
 */
GUI::GUI(lv_obj_t* parent, lv_color_t mainColor)
 ...
 */
void GUI::setStateCallback(std::function<void(state_t state)> callback) {
  stateFnc = callback;
}
 ...
 */
void GUI::setResetCallback(std::function<void()> callback) {
  resetFnc = callback;
}
 ...
 */
void GUI::setData(state_t state, sensors_t sensors) {
 ...
 */
lv_res_t GUI::tileAction(lv_obj_t* tileObj) {
  GUI* that = static_cast<GUI*>(lv_obj_get_free_ptr(tileObj));
 ...
 */
lv_res_t GUI::resetAction(lv_obj_t* btn) {
  GUI* that = static_cast<GUI*>(lv_obj_get_free_ptr(btn));
